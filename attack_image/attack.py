...
def get_loss(
    ...
    change_images: np.ndarray,
    change_images_label_probability: np.ndarray,
):
    """
    ...
    max_log_non_target_prob = np.log(
        np.max(change_images_label_probability[:, non_target_mask]) + LOSS_EPS
    )

    classification_loss = (
        np.log(change_images_label_probability[:, image_true_label])
...
def get_grad_in_1_pixel(
    model, cur_image, original_image, image_true_label: int, pixel_index_3d: np.ndarray
):
    ...
    grad_image_loss = get_loss(
        original_image.detach().numpy(),
        image_true_label,
        grad_images.detach().numpy(),
...
def single_step(
    ...
    for sample_index in sample_indices:
        grad_i = get_grad_in_1_pixel(
            model, last_image, original_image, true_label, sample_index
        )
        if grad_i is None:
            continue
        T_matrix[tuple(sample_index)] += 1
        M_matrix[tuple(sample_index)] = (
            BETA_1 * M_matrix[tuple(sample_index)] + (1 - BETA_1) * grad_i
        )
        V_matrix[tuple(sample_index)] = (
            BETA_2 * V_matrix[tuple(sample_index)] + (1 - BETA_2) * grad_i**2
        )
        ...
        pixel_value = next_image[tuple(sample_index)] + (
...
def attack(model, image, label):
    ...
    for i in range(100):
        ...
        cur_loss = get_loss(
            image.detach().numpy(),
            label,
            next_image.detach().numpy().reshape(1, *image_shape),
