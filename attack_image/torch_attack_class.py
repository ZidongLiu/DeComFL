...
class Attack:
    ...
    def __init__(
        self, model: nn.Module, hyper_params: HyperParams = DEFAULT_HYPER_PARAMS
    ):
        # hyper_params
    ...
    def _reset_adam_helper_matrices(self):
        self.T_matrix, self.M_matrix, self.V_matrix = (
            self._generate_adam_helper_matrices()
        )
    ...
    def sample_channel_height_width(self, channel_index: int):
        indices_1d = np.random.choice(
            self.indexes_to_be_sampled, self.sample_per_channel, replace=False
        )
        return np.stack(
            [
                channel_index * np.ones(self.sample_per_channel, dtype="int32"),
                indices_1d // self.width,
                indices_1d % self.width,
            ],
            axis=1,
        )
    ...
    def get_loss(
        self, change_images: torch.tensor, change_images_label_probability: torch.tensor
    ) -> torch.tensor:
        """
        ...
        classification_loss = (
            torch.log(change_images_label_probability[:, self.image_label])
    ...
    def get_grad_in_many_pixels(
        self, cur_image: torch.tensor, many_pixel_indices_3d: torch.tensor
    ) -> torch.tensor:
        self.check_image_data_valid()

        n_sample = len(many_pixel_indices_3d)
        many_pixel_indices_tuple = (
            many_pixel_indices_3d[:, 0],
        ...
        minus_indexes: tuple = (
            range(n_sample, n_sample * 2),
            *many_pixel_indices_tuple,
        )
        minus_tensor = grad_images[minus_indexes] - self.h

        plus_tensor[plus_tensor > self.image_value_upper_bound] = (
            self.image_value_upper_bound
        )
        minus_tensor[minus_tensor < self.image_value_lower_bound] = (
        ...
        sample_grads = (
            grad_image_losses[plus_indexes[0]] - grad_image_losses[minus_indexes[0]]
        ) / grad_step_size
    ...
    def single_step(self, last_image: torch.tensor) -> torch.tensor:
        ...
        BETA_1, BETA_2, learning_rate, ADAM_EPS = (
            self.BETA_1,
            self.BETA_2,
            self.learning_rate,
            self.ADAM_EPS,
        ...
        M_matrix[sample_indices_tuple] = (
            BETA_1 * M_matrix[sample_indices_tuple] + (1 - BETA_1) * grads
        )
        V_matrix[sample_indices_tuple] = (
            BETA_2 * V_matrix[sample_indices_tuple] + (1 - BETA_2) * grads**2
        )
        ...
        clamped_updates = torch.clamp(
            updated_pixel_values,
            self.image_value_lower_bound,
            self.image_value_upper_bound,
        )
        ...
        with torch.no_grad():
            ...
            for i in range(iteration):
                ...
                cur_loss = self.get_loss(
                    next_image.reshape(1, *self.image_shape), pred_prob.reshape(1, -1)
                )
