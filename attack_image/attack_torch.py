...
def sample_channel_height_width(channel_index):
    indices_1d = np.random.choice(
        indexes_to_be_sampled, sample_per_channel, replace=False
    )
    return np.stack(
        [
...
def get_loss(
    ...
    max_log_non_target_prob = torch.log(
        torch.max(change_images_label_probability[:, non_target_mask]) + LOSS_EPS
    )

    classification_loss = (
        torch.log(change_images_label_probability[:, image_true_label])
...
def get_grad_in_many_pixels(
    ...
    grad_image_losses = get_loss(
        original_image, image_true_label, grad_images, grad_images_probability
    )

    sample_grads = (
        grad_image_losses[plus_indexes] - grad_image_losses[minus_indexes]
    ) / grad_step_size
...
def single_step(
    ...
    grads = get_grad_in_many_pixels(
        model, last_image, original_image, true_label, sample_indices
    )
    ...
    clamped_updates = torch.clamp(
        updated_pixel_values, image_value_lower_bound, image_value_upper_bound
    )
...
def attack(model, image, label):
    ...
    success = False
    for i in range(100):
        next_image = single_step(
            T_matrix, M_matrix, V_matrix, model, last_image, image, label
        )
        ...
        if pred_label != label:
            ...
            break
        cur_loss = get_loss(
            image, label, next_image.reshape(1, *image_shape), pred_prob.reshape(1, -1)
        )
...
model, optimizer = get_model_and_optimizer(
    SimpleCNN(), checkpoint=simple_cnn_check_point
)
