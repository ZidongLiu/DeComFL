...
- **Run DeComFL:** Follow FL routine, split data into chunks and train on different clients.
  Usage example: `uv run decomfl_main.py --large-model=opt-125m --dataset=sst2 --iterations=1 --train-batch-size=8 --test-batch-size=200 --eval-iterations=25 --num-clients=1 --num-sample-clients=1 --local-update-steps=1 --num-pert=5 --lr=1e-5 --mu=1e-3 --grad-estimate-method=rge-forward --no-optim`
