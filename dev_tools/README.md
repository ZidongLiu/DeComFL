...
In root folder, install development dependencies using uv:
...
Use the uv virtual environment for this repo in VScode. This is needed for mypy to function.
...
NOTE: Replace `/path/to/your/uv-venv` with the actual path to your uv virtual environment.

Alternatively, you can get the path to python interpreter and ruff by running:

```
uv run which python # For windows try: uv run where python
uv run which ruff   # For windows try: uv run where ruff
```

Or find the uv virtual environment location:

```
uv venv --help  # Shows where uv creates virtual environments
```
...
```settings.json
{
	...
	"ruff.enable": true,
	"ruff.interpreter": ["/path/to/your/uv-venv/python"], # or ["/path/to/your/uv-venv/python.exe"] for Windows
	"ruff.path": ["/path/to/your/uv-venv/bin/ruff"], # or ["/path/to/your/uv-venv/Scripts/ruff.exe"] for Windows
}
```
