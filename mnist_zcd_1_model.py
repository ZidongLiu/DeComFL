...
from optimizers.zo_coordinate_descent import ZCD
...
if __name__ == "__main__":
    ...
    tensorboard_sub_folder = f"zcd-{grad_estimate_method}-{get_current_datetime_str()}"
    ...
    with tqdm(total=total_steps, desc="Training:") as t, torch.no_grad():
        for epoch_idx in range(n_epoch):
            for train_batch_idx, data in enumerate(train_loader):
                ...
                # update models
                grad = zcd.perturb_and_step(images, labels, model, criterion)
                # pred
                pred = model(images)
                #
