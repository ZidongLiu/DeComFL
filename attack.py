...
def single_step(T_matrix: np.ndarray, M_matrix: np.ndarray, V_matrix: np.ndarray, model: torch.nn.Module, last_image: torch.tensor, original_image: torch.tensor, true_label: int) -> torch.tensor:
    ...
    for sample_index in sample_indices:
        ...
        if grad_i is None:
            continue
        T_matrix[*sample_index] += 1
        M_matrix[*sample_index] = BETA_1 * M_matrix[*sample_index] + (1 - BETA_1) * grad_i
        V_matrix[*sample_index] = BETA_2 * V_matrix[*sample_index] + (1 - BETA_2) * grad_i ** 2
        
        T_i = T_matrix[*sample_index]
        M_i = M_matrix[*sample_index] / (1 - BETA_1 ** T_i )
        V_i = V_matrix[*sample_index] / (1 - BETA_2 ** T_i )
        
        pixel_value = next_image[*sample_index] + (-learning_rate * M_i / (V_i ** 0.5 + ADAM_EPS))
        ...
        next_image[*sample_index] = pixel_value
