...
DEFAULT_HYPER_PARAMS: HyperParams = HyperParams({
    ...
    1.0,
    'CLASSIFICATION_LOSS_MULTIPLIER':
    0.5,
    'N_LABEL':
    10,
})
...
class Attack:
    ...
    def __init__(self,
                 model: nn.Module,
                 hyper_params: HyperParams = DEFAULT_HYPER_PARAMS):
        ## hyper_params
        ...
        self.image_value_lower_bound: float = hyper_params[
            'image_value_lower_bound']
        self.image_value_upper_bound: float = hyper_params[
            'image_value_upper_bound']
        self.CLASSIFICATION_LOSS_MULTIPLIER: float = hyper_params[
            'CLASSIFICATION_LOSS_MULTIPLIER']
        ...
        self.indexes_to_be_sampled: np.ndarray = np.arange(self.height *
                                                           self.width)
    ...
    def _set_image_example_data(self,
    ...
    def sample_channel_height_width(self, channel_index: int):
        indices_1d = np.random.choice(self.indexes_to_be_sampled,
                                      self.sample_per_channel,
                                      replace=False)
        return np.stack([
            channel_index * np.ones(self.sample_per_channel, dtype='int32'),
            indices_1d // self.width, indices_1d % self.width
        ],
                        axis=1)
    ...
    def get_loss(
            self, change_images: torch.tensor,
            change_images_label_probability: torch.tensor) -> torch.tensor:
        '''
        ...
        classification_loss = torch.log(
            change_images_label_probability[:, self.image_label]
    ...
    def get_grad_in_many_pixels(
            self, cur_image: torch.tensor,
            many_pixel_indices_3d: torch.tensor) -> torch.tensor:
        self.check_image_data_valid()

        n_sample = len(many_pixel_indices_3d)
        many_pixel_indices_tuple = (many_pixel_indices_3d[:, 0],
                                    many_pixel_indices_3d[:, 1],
                                    many_pixel_indices_3d[:, 2])
        grad_images = torch.clone(cur_image).unsqueeze(0).repeat(
            n_sample * 2, 1, 1, 1)
        plus_indexes = range(n_sample)
        plus_tensor = grad_images[plus_indexes,
                                  *many_pixel_indices_tuple] + self.h

        minus_indexes = range(n_sample, n_sample * 2)
        minus_tensor = grad_images[minus_indexes,
                                   *many_pixel_indices_tuple] - self.h

        plus_tensor[plus_tensor > self.
                    image_value_upper_bound] = self.image_value_upper_bound
        minus_tensor[minus_tensor < self.
                     image_value_lower_bound] = self.image_value_lower_bound
        grad_images[plus_indexes, *many_pixel_indices_tuple] = plus_tensor
        grad_images[minus_indexes, *many_pixel_indices_tuple] = minus_tensor
    ...
    def single_step(self, last_image: torch.tensor) -> torch.tensor:
        ...
        clamped_updates = torch.clamp(updated_pixel_values,
                                      self.image_value_lower_bound,
                                      self.image_value_upper_bound)
        ...
        with torch.no_grad():
            ...
            for i in range(iteration):
                ...
                cur_loss = self.get_loss(
                    next_image.reshape(1, *self.image_shape),
